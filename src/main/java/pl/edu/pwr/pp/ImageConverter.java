...
public class ImageConverter {
	...
	public enum ConvertType {
		Low("Niska"), LowNegative("Niska (odwrócona)"), High("Wysoka"), HighNegative("Wysoka (odwrócona)");
	...
	public enum ScaleType {
		Signs_80("80 znaków"), Signs_160("160 znaków"), Screen_width("Szerokość ekranu"), Not_scaled(
				"Oryginał (bez skalowania)"), Custom("Niestandardowa szerokość...");
	...
	public static char IntToAsciiLow(int intensity) {
		return INTENSITY_2_ASCII_10
				.charAt((int) ((double) intensity / (double) ((double) 256 / (double) INTENSITY_2_ASCII_10.length())));
	}

	public static char IntToAsciiLowNegative(int intensity) {
		return INTENSITY_2_ASCII_10.charAt((INTENSITY_2_ASCII_10.length() - 1)
				- (int) ((double) intensity / (double) ((double) 256 / (double) INTENSITY_2_ASCII_10.length())));
	}

	public static char IntToAsciiHigh(int intensity) {
		return INTENSITY_2_ASCII_70
				.charAt((int) ((double) intensity / (double) ((double) 256 / (double) INTENSITY_2_ASCII_70.length())));
	}

	public static char IntToAsciiHighNegativ(int intensity) {
		return INTENSITY_2_ASCII_70.charAt((INTENSITY_2_ASCII_70.length() - 1)
				- (int) ((double) intensity / (double) ((double) 256 / (double) INTENSITY_2_ASCII_70.length())));
	}
	...
	public static char[][] intensitiesToAscii(int[][] intensities, ConvertType type) {
		...
		IntStream.range(0, rows).forEach(y -> IntStream.range(0, columns).forEach(x -> {
			switch (type) {
			case Low:
		...
		for (int y = 0; y < image.getHeight(); y++)
			for (int x = 0; x < image.getWidth(); x++) {
				...
				raster.setSample(x, y, 0,
						(int) ((0.2989 * color.getRed()) + (0.5870 * color.getGreen()) + (0.1140 * color.getBlue())));
			}
